...
function cargar(){
  ...
  try {
    ...
    ["sesiones","pesos","nutricion","supl","ajustes","perfil"].forEach(function(k){
...
function renderEntreno(dia){
  ...
  $("#banda").innerHTML =
    '<div class="fila"><div><h2>Día ' + diaActual + ' · ' + esc(D.titulo) + '</h2>' +
  ...
  $("#btnreset").addEventListener("click", function(){
    if (!confirm("¿Borrar las series registradas hoy en el día " + diaActual + "?")) return;
    var s = sesion();
    ...
    '<article class="ej" id="card-' + esc(e.id) + '">' +
      '<div class="ej-top">' +
        ...
        '<div class="ej-info">' +
          ...
          '<div class="tags"><span class="tag tipo">' + esc(e.tipo) + '</span><span class="tag">' + esc(e.eq) + '</span><span class="tag">' + esc(e.rir) + '</span><span class="tag">' + esc(e.sr) + '</span></div>' +
...
/* Objetivos derivados del peso corporal — guía §5.1 y §5.2.
   La tabla de la guía es lineal: 70 kg → 2.900 kcal, +20 kcal por kg. */
function objetivos(peso){
  var p = Math.min(150, Math.max(45, peso));
  var kcal = Math.round((2900 + (p - 70) * 20) / 10) * 10;
  var prot = Math.round(p * 1.8);                       /* 1,6-1,8 g/kg, apuntamos al alto */
  var gras = Math.max(60, Math.round(kcal * 0.25 / 9)); /* ~25 % kcal, suelo de 60 g */
  var carb = Math.max(0, Math.round((kcal - prot * 4 - gras * 9) / 4));
  return { kcal:kcal, prot:prot, gras:gras, carb:carb };
}
...
function renderNutricion(){
  ...
  html += '<div class="bloque"><h3>Reparto que buscas</h3>' +
    '<p class="desc">3-4 comidas con 30-40 g de proteína cada una. El total diario manda muchísimo más que el timing; ' +
    'lo único que merece la pena: carbos 1-2 h antes de entrenar y una comida completa después.</p></div>';
...
/* Media de las últimas 4 semanas. La guía insiste: ninguna decisión con el
   peso de un solo día, que baila 1-2 kg por agua. */
...
function ritmoMensual(){
  ...
  return reciente - previa;                    /* kg por 4 semanas ≈ kg/mes */
}
...
function escenarioActivo(){
  ...
  if (r < 0.1) return 1;
  if (r > 1.0) return 2;
  return null;                                  /* dentro del rango objetivo */
}

function sparkline(pts){
  if (pts.length < 2) return "";
  var w = 320, h = 70, pad = 6;
  var ks = pts.map(function(p){ return p.kg; });
...
function renderProgreso(){
  ...
  var esc_ = escenarioActivo();

  var html =
    '<div class="bloque"><h3>Peso corporal</h3>' +
    '<p class="desc">Ayunas, misma hora, misma báscula. Las decisiones se toman con la media de 4 semanas, nunca con el número de un día.</p>' +
    '<div class="kpis">' +
  ...
  if (ritmo != null){
    var dentro = ritmo >= 0.25 && ritmo <= 0.5;
    html += '<div class="esc' + (dentro ? "" : " activo") + '" id="ritmobox" style="margin-top:12px">' +
      '<h4>' + (dentro ? "Ritmo correcto" : "Ritmo fuera de objetivo") + '</h4>' +
      '<div class="cond">Objetivo: +0,25 a +0,5 kg/mes</div>' +
      (dentro ? "Vas justo donde toca. No toques nada." :
        (ritmo < 0.25 ? "Estás por debajo del objetivo." : "Estás por encima: parte de esa ganancia será grasa.")) +
      '</div>';
  } else {
  ...
  html += '<div class="bloque"><h3>Cintura</h3>' +
    '<p class="desc">Cada 4 semanas, cinta a la altura del ombligo, relajado. Si crece más de 2-3 cm, revisa el escenario 5.</p>' +
    (ultM ? '<div class="kpis"><div class="kpi"><div class="v">' + decr(ultM.cintura, 1) + '</div><div class="l">Última<br>cm</div></div>' +
            '<div class="kpi"><div class="v">' + fmtFecha(ultM.d) + '</div><div class="l">Fecha</div></div></div>' : '') +
    '<div class="entrada"><input type="text" inputmode="decimal" id="nuevaCintura" placeholder="cm" aria-label="Cintura en cm">' +
    '<button id="addCintura">Apuntar</button></div></div>';
  ...
  html += '<div class="bloque"><h3>Fotos de progreso</h3>' +
    '<p class="desc">Frente, lado y espalda cada 4 semanas. Misma luz, misma hora, mismo sitio. El espejo diario no detecta cambios graduales; las fotos sí.</p>' +
    (diasFoto == null
  ...
  html += '<div class="bloque"><h3>Árbol de decisión</h3>' +
    '<p class="desc">Se evalúa cada 4 semanas, nunca antes. Ninguna decisión con menos de 3-4 semanas de datos.</p>';
  window.ESCENARIOS.forEach(function(e){
    html += '<div class="esc' + (esc_ === e.id ? " activo" : "") + '" style="margin-bottom:8px">' +
      '<h4>' + (esc_ === e.id ? "▶ " : "") + e.id + ". " + esc(e.t) + '</h4>' +
      '<div class="cond">' + e.cond + '</div>' + e.txt + '</div>';
  });
  html += '</div>';

  /* Historial de entrenos */
  var fechas = Object.keys(S.sesiones).filter(function(f){
    var s = S.sesiones[f];
    return Object.keys(s.ej).some(function(id){ return s.ej[id].s && s.ej[id].s.some(Boolean); });
  ...
  if (!fechas.length) html += '<p class="gnota">Todavía no has registrado ninguna serie.</p>';
    ...
    fechas.forEach(function(f){
      var s = S.sesiones[f], n = 0;
      Object.keys(s.ej).forEach(function(id){ n += (s.ej[id].s || []).filter(Boolean).length; });
      html += '<li><span>' + fmtFecha(f) + ' · Día ' + esc(s.dia || "?") + '</span><b>' + n + ' series</b></li>';
    });
    html += '</ul>';
  }
  html += '</div>';
  ...
  html += '<div class="bloque"><h3>Tus datos</h3>' +
    '<p class="desc">Todo vive solo en este teléfono. Si borras los datos de Safari o cambias de móvil, se pierde. ' +
    'Haz una copia de vez en cuando.</p>' +
  ...
  $("#addPeso").addEventListener("click", function(){
    ...
    if (kg == null || kg < 30 || kg > 250){ toast("Introduce un peso válido en kg."); return; }
    var f = hoy();
    var yaHoy = S.corporal.filter(function(p){ return p.d === f; })[0];
  ...
  $("#addCintura").addEventListener("click", function(){
    var cm = num($("#nuevaCintura").value);
    if (cm == null || cm < 40 || cm > 200){ toast("Introduce una medida válida en cm."); return; }
    S.medidas.push({ d: hoy(), cintura: cm });
    S.medidas.sort(function(a, b){ return a.d < b.d ? -1 : 1; });
    guardar(); renderProgreso();
    toast("Cintura apuntada.");
  });
...
function irA(tab){
  ...
  var titulos = {
    entreno:   ["Rutina 4 días", "Torso · Pierna · ×2 por músculo"],
    nutricion: ["Nutrición", "Superávit ligero · proteína alta"],
    progreso:  ["Progreso", "Decisiones con 4 semanas de datos"],
